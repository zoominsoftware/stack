...
data PantryWireException
  ...
  | UnexpectedSha256 !StaticSHA256
                     !StaticSHA256
  | TruncatedInputForHash !StaticSHA256
                          !Int
                          !Int
  | SHA256Mismatch !StaticSHA256
                   !StaticSHA256
...
-- PantryWireException.
wireReceiverConduit ::
     MonadThrow m => [(StaticSHA256, Int)] -> ConduitM ByteString WireEvent m ()
  ...
  where
          ...
          case result of
            ...
            Right givenSha256
              | givenSha256 == expectedSha256 -> do
                ...
                actualSha256 <- sizeValidatingConduit givenSha256 expectedLength
                if actualSha256 == givenSha256
                  then go following
                  else throwM (SHA256Mismatch givenSha256 actualSha256)
              | otherwise -> throwM (UnexpectedSha256 givenSha256 givenSha256)
    sizeValidatingConduit expectedSha256 expectedLength = do
      ...
      if len == expectedLength
        ...
        else throwM (TruncatedInputForHash expectedSha256 expectedLength len)

-- | Send wire events to the pantry client.
wireSenderConduit :: PrimMonad m => ConduitM WireEvent ByteString m ()
...
pairToEventConduit ::
     Monad m => ConduitM (StaticSHA256, ByteString) WireEvent m ()
pairToEventConduit =
